...
def extract_multiple_aligned(
    imgs: list, levels: int = 4, sigma: float = 5, processes: int = None, batch_size=cpu_count(), tqdm_str: str = ""
) -> np.ndarray:
    """
...
def saturation(im: np.ndarray) -> np.ndarray:
    ...
    satur_map = np.bitwise_not(
        np.bitwise_and(
            np.bitwise_and(np.bitwise_and(im_h != 0, im_v != 0), np.roll(im_h, (0, -1), (0, 1)) != 0),
    ...
    for ch_idx, max_c in enumerate(max_ch):
        if max_c > 250:
            satur_map[:, :, ch_idx] = np.bitwise_not(np.bitwise_and(im[:, :, ch_idx] == max_c, satur_map[:, :, ch_idx]))
